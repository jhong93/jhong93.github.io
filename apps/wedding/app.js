(() => {
  ...
  const lockedPageRegions = [skipLink, pageMain, lightbox];
  ...
  function updateLoadedStatus() {
    ...
    setDecryptStatus(
      failed
        ? `${failed} photographs could not load.`
        : loaded === total
        ? ""
        : `Loading photographs · ${loaded} / ${total}`,
      failed > 0,
    );
  }
  ...
  async function attachPhoto(img, photo) {
    ...
    const fallback = shell?.querySelector(".photo-shell__fallback");
    if (fallback) fallback.textContent = "Loading photograph…";
    ...
    try {
      ...
      if (fallback) fallback.textContent = "Photograph unavailable";
  ...
  function buildPhotoCard(photo, index) {
    ...
    fallback.textContent = "Loading photograph…";
  ...
  function showLightboxPhoto() {
    ...
    lightboxPhoto.alt = `Wedding photograph ${lightboxIndex + 1} of ${activePhotos.length}`;
